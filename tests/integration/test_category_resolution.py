...
class TestCategoryResolution:
    ...
    async def test_category_assignment_structure(self, real_db_manager):
        ...
        for sample in samples[:3]:
            print(f"  Transaction: {sample.get('ZTRANSACTION')}, Category: {sample.get('ZCATEGORY')}")
    ...
    async def test_category_names_table(self, real_db_manager):
        ...
        for cat in categories[:5]:
            print(f"  ID: {cat.get('Z_PK')}, ZNAME: {cat.get('ZNAME')}, ZNAME2: {cat.get('ZNAME2')}")
    ...
    async def test_transaction_service_category_resolution(self, real_db_manager):
        ...
        for raw_tx in raw_transactions[:5]:
            ...
            transaction = TransactionModel(
                ...
                account_id=str(raw_tx["ZACCOUNT2"]) if raw_tx["ZACCOUNT2"] else "unknown",
                amount=Decimal(str(raw_tx["ZAMOUNT1"])) if raw_tx["ZAMOUNT1"] else Decimal("0"),
            ...
            enhanced = await transaction_service._enhance_transaction_with_metadata(transaction)
        ...
        if categorized_count == 0:
            ...
            print("This explains why savings recommendations show all spending as uncategorized.")
        else:
    ...
    async def test_alternative_category_resolution_strategies(self, real_db_manager):
        ...
        if any(t['name'] == 'ZCATEGORYASSIGNMENT' for t in category_tables):
            ...
            try:
                ...
                print(f"Alternative table results: {alt_categorized}/{len(alt_results)} categorized")
            except Exception as e:
