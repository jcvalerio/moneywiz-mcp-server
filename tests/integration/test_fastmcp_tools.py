...
class TestFastMCPToolsIntegration:
    ...
    async def test_get_savings_recommendations_tool(self, setup_mcp_config):
        ...
        with patch('moneywiz_mcp_server.main.get_db_manager') as mock_get_db:
            ...
            with patch('moneywiz_mcp_server.main.SavingsService') as mock_service_class:
                ...
                mock_service.get_savings_recommendations.return_value = mock_savings_data
    ...
    async def test_analyze_spending_trends_tool(self, setup_mcp_config):
        ...
        with patch('moneywiz_mcp_server.main.get_db_manager') as mock_get_db:
            ...
            mock_trend_data = {
                ...
                "visualizations": {
                    "line_chart": {"labels": ["2024-01"], "datasets": [{"data": [3500.0]}]}
                }
            }
    ...
    async def test_analyze_income_expense_trends_tool(self, setup_mcp_config):
        ...
        with patch('moneywiz_mcp_server.main.get_db_manager') as mock_get_db:
            ...
            with patch('moneywiz_mcp_server.main.TrendService') as mock_service_class:
                mock_service = AsyncMock()
                mock_service.analyze_income_vs_expense_trends.return_value = mock_income_expense_data
                mock_service_class.return_value = mock_service
            ...
            # Verify service call
            mock_service.analyze_income_vs_expense_trends.assert_called_once_with(months=12)
    ...
    async def test_database_connection_lifecycle(self, setup_mcp_config):
        ...
        with patch('moneywiz_mcp_server.main.get_db_manager') as mock_get_db:
            ...
            with patch('moneywiz_mcp_server.main.SavingsService') as mock_service_class:
                mock_service = AsyncMock()
                mock_service.get_savings_recommendations.side_effect = Exception("Test error")
                mock_service_class.return_value = mock_service
                
                # Act & Assert - should raise the exception
                with pytest.raises(RuntimeError, match="Failed to generate savings recommendations"):
