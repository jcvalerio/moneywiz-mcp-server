...
from ..utils.validators import validate_account_type, validate_account_id
from ..utils.formatters import format_currency

logger = logging.getLogger(__name__)


async def calculate_account_balance(db_manager: DatabaseManager, account_pk: int) -> float:
    ...
    try:
        ...
        transaction_total = sum(float(txn.get('ZAMOUNT1', 0) or 0) for txn in transactions)
        
        # MoneyWiz formula: opening_balance + sum(transaction_amounts)
        final_balance = opening_balance + transaction_total
        
        logger.debug(f"Account {account_pk}: opening={opening_balance}, transactions={transaction_total}, final={final_balance}")
...
def list_accounts_tool(db_manager: DatabaseManager) -> Tool:
    ...
    async def handler(
        ...
        """
        logger.info(f"Listing accounts (include_hidden={include_hidden}, type={account_type})")
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "account_type": {
                    ...
                    "enum": ["checking", "savings", "credit_card", "investment", "cash", "loan", "property"]
                }
            }
        },
        handler=handler
...
def get_account_tool(db_manager: DatabaseManager) -> Tool:
    ...
    async def handler(
        ...
        Raises:
            ...
            RuntimeError: If database operations fail
        """
        logger.info(f"Getting account details for {account_id} (include_txns={include_transactions})")
        ...
        try:
            ...
            for entity_id in account_entities:
                ...
                try:
                    ...
                    results = await db_manager.execute_query(query, (entity_id, account_id, pk_value))
                    if results:
                        account = results[0]
                        entity_name = entity_types.get(entity_id, 'unknown')
                        break
                    ...
                    results = await db_manager.execute_query(query, (entity_id, account_id))
                    if results:
                        account = results[0]
                        entity_name = entity_types.get(entity_id, 'unknown')
                        break
            ...
            if include_transactions:
                ...
                try:
                    ...
                    result['transactions_note'] = "Transaction history integration coming soon"
                    
                except Exception as e:
                    logger.warning(f"Failed to get transactions for account {account_id}: {e}")
