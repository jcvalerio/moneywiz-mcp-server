        ...
        try:
            ...
            for transaction in transactions:
                formatted_transaction = {
                    ...
                    "amount": format_currency(float(transaction.amount), transaction.currency),
                ...
                if transaction.original_currency and transaction.original_currency != transaction.currency:
                    formatted_transaction["original_amount"] = format_currency(
                        float(transaction.original_amount or 0), 
                        transaction.original_currency
                    )
                    formatted_transaction["original_currency"] = transaction.original_currency
                
                result.append(formatted_transaction)
            
            logger.info(f"Found {len(result)} transactions for {format_date_range_for_display(date_range)}")
            return result
        ...
        try:
            ...
            result = {
                ...
                "summary": {
                    ...
                    "average_per_category": format_currency(
                        float(total_expenses / len(categories)) if categories else 0, "USD"
                    )
                }
            }
            ...
            for i, category in enumerate(categories, 1):
                category_data = {
                    ...
                    "total_amount": format_currency(float(category.total_amount), "USD"),
                    "transaction_count": category.transaction_count,
                    "average_amount": format_currency(float(category.average_amount), "USD"),
                    "percentage_of_total": round(category.percentage_of_total, 1),
                    "impact_level": "high" if category.percentage_of_total > 20 else 
                                  "medium" if category.percentage_of_total > 10 else "low"
                }
            ...
            if categories:
                ...
                result["insights"] = {
                    ...
                    "highest_impact_amount": format_currency(float(top_category.total_amount), "USD"),
                    "highest_impact_percentage": round(top_category.percentage_of_total, 1),
    ...
    async def handler(
        ...
        try:
            ...
            result = {
                "analysis_period": format_date_range_for_display(date_range),
                "financial_overview": {
                    "total_income": format_currency(float(analysis.total_income), analysis.currency),
                    "total_expenses": format_currency(float(analysis.total_expenses), analysis.currency),
                    "net_savings": format_currency(float(analysis.net_savings), analysis.currency),
                ...
                "savings_analysis": {
                    ...
                    "monthly_savings": format_currency(float(analysis.net_savings / 3), analysis.currency),  # Assuming 3 months
                    "recommendations": []
                }
            }
            ...
            for category in analysis.expense_breakdown[:10]:  # Top 10
                result["expense_breakdown"].append({
                    "category": category.category_name,
                    "amount": format_currency(float(category.total_amount), analysis.currency),
            ...
            logger.info(f"Generated income vs expense analysis with {analysis.savings_rate:.1f}% savings rate")
            return result
