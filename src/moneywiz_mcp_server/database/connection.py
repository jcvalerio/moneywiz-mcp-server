...
class DatabaseManager:
    ...
    def __init__(self, db_path: str, read_only: bool = True) -> None:
        ...
        self._api: Optional[Any] = None  # MoneywizApi instance
        self._connection: Optional[aiosqlite.Connection] = None
        
        logger.info(f"DatabaseManager initialized for {db_path} (read_only={read_only})")
    ...
    async def initialize(self) -> None:
        ...
        if MoneywizApi is None:
            logger.warning("moneywiz-api library not found. Using direct SQLite access only.")
            self._api = None
        else:
            try:
        ...
        try:
            ...
            await self._connection.execute("PRAGMA query_only = ON" if self.read_only else "PRAGMA query_only = OFF")
    ...
    def api(self) -> Any:
        ...
        if self._api is None:
            raise RuntimeError("MoneywizApi not available. Using direct SQLite access only.")
        return self._api
